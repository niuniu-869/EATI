...
export function renderResult(result, userLevels, dimOrder, dimDefs, config) {
  ...
  else if (mode === "fallback") kicker.textContent = "系统 404 兜底";
  else kicker.textContent = "你的主食格";
  ...
  document.getElementById("result-badge").textContent =
    ...
    (primary.exact != null
      ? ` · 精准命中 ${primary.exact}/${totalDims} 维`
      : "");
...
${siteUrl}`;
    try {
      ...
      setTimeout(() => {
        btnCopy.textContent = "复制朋友圈文案";
        copyHint.style.display = "none";
      }, 2400);
    } catch (e) {
      btnCopy.textContent = "复制失败，请手动选中";
    }
  };
}
