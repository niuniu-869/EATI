...
export async function generateShareImage(primary, userLevels, dimOrder, dimDefs, mode, config) {
  ...
  const kickerText = mode === 'hotpot' ? '火锅教祖已激活' : mode === 'fallback' ? '系统 404 兜底' : '你的主食格'
  ...
  const badgeText =
    ...
    (primary.exact != null ? ` · 精准命中 ${primary.exact}/${totalDims} 维` : '')
  ...
  for (const dim of dimOrder) {
    ...
    ctx.fillStyle = val === 3 ? THEME.high : val === 2 ? THEME.mid : THEME.low
    ctx.fill()
  ...
  ctx.fillStyle = THEME.textMute
  ctx.fillText('EATI 食格测试 · 厦门大学美食协会出品', W / 2, H - cardY - 38)
  ctx.font = '400 13px system-ui, "PingFang SC", "Microsoft YaHei", sans-serif'
  ctx.fillStyle = '#b5a598'
  ctx.fillText('仅供娱乐｜一个吃遍厦门的学生社团', W / 2, H - cardY - 18)

  // 下载
  const link = document.createElement('a')
  link.download = `EATI-${primary.code}.png`
...
function drawShareRadar(ctx, cx, cy, maxR, userLevels, dimOrder, dimDefs) {
  ...
  for (let lv = 3; lv >= 1; lv--) {
    ...
    ctx.fillStyle =
      lv === 3 ? 'rgba(194,59,34,0.05)' : lv === 2 ? 'rgba(194,59,34,0.035)' : 'rgba(194,59,34,0.02)'
    ctx.fill()
  ...
  for (let i = 0; i < n; i++) {
    ...
    ctx.moveTo(cx, cy)
    ctx.lineTo(x, y)
    ctx.strokeStyle = 'rgba(194,59,34,0.1)'
    ctx.lineWidth = 0.5
    ctx.stroke()
  ...
  ctx.closePath()
  ctx.fillStyle = 'rgba(194,59,34,0.2)'
  ctx.fill()
  ctx.strokeStyle = 'rgba(194,59,34,0.7)'
  ctx.lineWidth = 2
  ctx.stroke()
